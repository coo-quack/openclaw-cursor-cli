...
- `cursor-cli/<model>` (for example `cursor-cli/grok-4.5-high-fast`) — the
  safe, **text-only** default. No OpenClaw tools are exposed to the model.
- `cursor-mcp/<model>` (for example `cursor-mcp/grok-4.5-high-fast`) — same
...
`cursor-cli/grok-4.5-high-fast` is the recommended default: fast, cheap on
...
To add other Cursor models (e.g. `cursor-cli/grok-4.5-high`,
...
bridge, e.g. `/model cursor-mcp/grok-4.5-high-fast`. No env var, no gateway
